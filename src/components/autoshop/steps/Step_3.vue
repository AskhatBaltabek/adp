...
export default {
  ...
  mounted() {
    this.$store.dispatch('autoshop/LOAD_DOCUMENT_TYPES')
  },
  methods: {
    ...
    async add() {
      if(this.validAll()) {
        this.data.push({is_holder: true})
      }
    },
    async downloadFile(type) {
      let result = await this.$store.dispatch('autoshop/GET_TEMPLATE', {policy_number: this.policy_number, type})
    ...
    onSearchClient(value) {
      this.$store.dispatch('autoshop/SEARCH_CLIENT', {iin: value})
        .then(resp => {
    ...
    calculate() {
      if (this.validAll()) {
        this.$store.dispatch('autoshop/SAVE_STEP_DATA', {data: this.data})
          .then(() => {
            this.$store.dispatch('autoshop/GET_PREMIUM', cloneDeep(this.data))
              .then(resp => {
    ...
    async cancelPolicy() {
      let result = await this.$store.dispatch('autoshop/SET_CANCELLATION', this.policy_number);
    ...
    setPolicy(isDraft) {
      if (this.validAll()) {
        this.$store.commit('autoshop/SET_DATA', {is_draft: isDraft})
        if(this.$route.params.id !== 'new') {
          this.$store.commit('autoshop/SET_DATA', {policy_id: this.$route.params.id})
        }
        this.$store.dispatch('autoshop/SAVE_STEP_DATA', {data: this.data})
            .then(async () => {

              let resp = await this.$store.dispatch('autoshop/SET_POLICY')

              if(resp.success) {
                await this.$message.success(resp.message)
                await this.$router.push({path: `/autoshop/${resp.data.id}`})
              } else {
  ...
  computed: {
    ...
    ...mapGetters('autoshop', ['policyCanceled', 'policyDone', 'policyDraft']),
    ...mapState('autoshop', {
