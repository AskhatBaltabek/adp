...
export default {
  ...
  methods: {
    ...
    onSearchClient(value) {
      this.$store.dispatch('mst/SEARCH_CLIENT', {iin: value, natural_person_bool: 1, resident_bool: this.data.resident_bool ? 1 : 0})
        .then(resp => {
            ...
            .then(() => {
              if(next > this.current) {
                if(this.data.customer_is_holder) {
                  ...
                  data.is_holder = true
                  this.$store.dispatch('mst/SAVE_STEP_DATA', {data: [data], step: next})
                }
                else this.$store.dispatch('mst/SAVE_STEP_DATA', {data: [{is_holder: true}], step: next})
              }
            })
            .then(() => {
